...
@app.post("/recipes/ask")
def recipe_ask(req: ChatQuestionRequest):
    """
    Answer a user question ONLY about a specific recipe identified by source_url.
    If the question is unrelated, the model is instructed to respond with a refusal.
    ...
    # Build a compact, explicit context block for the model
    def secs_to_text(val: Optional[int]) -> str:
        try:
            v = int(val or 0)
        except Exception:
            v = 0
    ...
    # Normalize steps to sectioned list
    ...
    # Create readable text context
    lines: List[str] = []
    lines.append(f"Title: {title or ''}")
    lines.append(f"Author: {author or ''}")
    lines.append(f"Servings: {servings or ''}")
    lines.append(f"Prep time: {secs_to_text(prep_time)} | Cook time: {secs_to_text(cook_time)} | Total time: {secs_to_text(total_time)}")
    lines.append("")
